...
RSpec.describe 'Hash Exercises' do
  ...
  describe 'update favorite movie exercise' do
    it 'returns hash with a new key/value pair when not included' do
      ...
      expect(update_favorite_movie(my_favorites, 'Avengers: Endgame')).to eq(
        result
      )
    end
    ...
    it 'returns hash with an updated key/value pair when included' do
      ...
      expect(
        update_favorite_movie(my_favorites, 'Avengers: Infinity War')
      ).to eq(result)
    end
  end
